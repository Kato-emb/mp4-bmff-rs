...
impl TkhdBox {
    ...
    /// Writes the `TkhdBox` to the given `WriteCursor`.
    pub fn write(&self, cur: &mut WriteCursor) -> Result<()> {
        let at = |e: ErrorKind, cur: &WriteCursor| Error::new(e).at(cur.position() as u64);
        let full_box_header = FullBoxHeader::new(self.version, self.flags);
        ...
        match self.version {
            0 => {
                ...
                cur.reserve_zeros(Self::RESERVED_1_SIZE)
                    .map_err(|e| at(e.into(), cur))?; // reserved
        ...
        for &m in self.matrix.to_raw().iter() {
            cur.write_i32_be(m).map_err(|e| at(e.into(), cur))?;
        }
